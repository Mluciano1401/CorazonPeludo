...
export class TipoPersonaController {
  userService: TipoPersonaService;
  constructor(private TipoPersonaService: TipoPersonaService) {
    this.userService = this.TipoPersonaService;
  }
  ...
  @Post('/update/:id')
  update(@Body() body:TipoPersona, @Param('id') id){
  ...
  @Post('/activar/:id')
  activar(@Body() body:TipoPersona, @Param('id') id){
    return this.userService. update(body).then(res=>{
      return {success: true, data: res}
    }).catch(error=>{
      throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR)
  ...
  @Post('/desactivar/:id')
  desactivar(@Body() body:TipoPersona, @Param('id') id){
    return this.userService. update(body).then(res=>{
      return {success: true, data: res}
    }).catch(error=>{
      throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR)
