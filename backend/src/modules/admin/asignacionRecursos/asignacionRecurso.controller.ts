...
export class AsignacionRecursosController {
  userService: AsignacionRecursosService;
  constructor(private AsignacionRecursosService: AsignacionRecursosService) {
    this.userService = this.AsignacionRecursosService;
  }
  ...
  @Post()
  save(@Body() body:AsignacionRecursos){
  ...
  @Post('/update/:id')
  update(@Body() body:AsignacionRecursos, @Param('id') id){
  ...
  @Post('/activar/:id')
  activar(@Body() body:AsignacionRecursos, @Param('id') id){
    return this.userService.update(id,body).then(res=>{
      return {success: true, data: res}
    }).catch(error=>{
      throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR)
  ...
  @Post('/desactivar/:id')
  desactivar(@Body() body:AsignacionRecursos, @Param('id') id){
    return this.userService.update(id,body).then(res=>{
      return {success: true, data: res}
    }).catch(error=>{
      throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR)
