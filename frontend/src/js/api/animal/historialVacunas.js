     ...
     async function populateTable() {
      try {
        ...
        data.data.forEach(item => {
          ...
          <button id="editar-${item.id}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/enums/registrohistorialvacunas.html?id${item.id}">Editar</a></button>
...
tableBody.addEventListener('click', (event) => {
  if (event.target.id.startsWith('eliminar-')) {
    ...
    if (confirm(`¿Está seguro de eliminar al usuario con ID ${userId}?`)) {
      ...
      fetch(`http://localhost:3000/historialvacunas/delete/${userId}`, {
        method: 'GET',
      })
        .then(response => {
          if (response.ok) {
