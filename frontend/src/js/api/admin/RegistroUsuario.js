...
async function getData(){
  ...
  .then(data => {
    ...
    select.addEventListener('change', () => {
      ...
      const selectedData = data.data.find(item => item.tipoUsuarioId === selectedId); // Find selected item
      if (selectedData) {
...
function populateSelectOptions(data) {
  ...
  data.data.forEach(item => {
    ...
    option.value = item.tipoUsuarioId; // Replace with your data item's ID property
