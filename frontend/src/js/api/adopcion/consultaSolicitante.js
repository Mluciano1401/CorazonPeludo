...
document.addEventListener('DOMContentLoaded', async() => {
  await populateTable();
     ...
     async function populateTable() {
      try {
        const response = await fetch('http://localhost:3000/solicitante'); // Replace with your URL
        ...
        data.data.forEach(item => {
          ...
          <button id="editar-${item.id}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/enums/registrosolicitante.html?id${item.id}">Editar</a></button>
...
async function busqueda(){
  const urlAPI = "http://localhost:3000/solicitante"; // URL de la API
...
await fetch(urlAPI)
  ...
  .then(data => {
    let solicitantes = data.data; // Copia del array original

    buscador.addEventListener('input', () => {
      const texto = buscador.value.toLowerCase();
      const solicitantesFiltrados = solicitantes.filter(solicitante => {
        return (
          solicitante.descripcion.toLowerCase().includes(texto)
        );
      });

      tablaCuerpo.innerHTML = ''; // Vaciar el cuerpo de la tabla
      solicitantesFiltrados.forEach(item => {
        const row = document.createElement('tr');
...
tableBody.addEventListener('click', (event) => {
    if (event.target.id.startsWith('editar-')) {
      ...
      window.location.href = `../../../../../../frontend/public/admin/registrosolicitante.html?id=${userId}`;
  ...
  tableBody.addEventListener('click', (event) => {
    if (event.target.id.startsWith('eliminar-')) {
      ...
      if (confirm(`¿Está seguro de eliminar al solicitante con ID ${userId}?`)) {
        // Logic to delete user (replace with your API call)
        fetch(`http://localhost:3000/solicitante/delete/${userId}`, {
          method: 'GET',
        })
          .then(response => {
            if (response.ok) {
