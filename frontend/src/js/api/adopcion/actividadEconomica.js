...
form.addEventListener('submit', async (event) =>{
  ...
  let url = '';
  if(id){
    url = 'http://localhost:3000/actividadeconomica/update/';
  }else{
    url = 'http://localhost:3000/actividadeconomica';
  }
   const data = {  
      id: id ? id : null,
...
async function getData(){
  await fetch(`http://localhost:3000/actividadeconomica/${id}`) // Replace with your actual backend URL
