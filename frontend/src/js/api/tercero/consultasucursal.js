...
document.addEventListener('DOMContentLoaded', async() => {
  await populateTable();
     ...
     async function populateTable() {
      try {
        const response = await fetch('http://localhost:3000/sucursal'); // Replace with your URL
        ...
        data.data.forEach(item => {
          ...
          <button id="editar-${item.id}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/enums/registrosucursal.html?id${item.id}">Editar</a></button>
...
async function busqueda(){
  const urlAPI = "http://localhost:3000/sucursal"; // URL de la API
    ...
    buscador.addEventListener('input', () => {
      ...
      usuariosFiltrados.forEach(item => {
        const row = document.createElement('tr');
          ...
          ${(item.status) ? `<button id="deshabilitar-${item.id}" class="btn btn-warning btn-sm me-1">Deshabilitar</button>` 
          : `<button id="habilitar-${item.id}" class="btn btn-success btn-sm me-1">Habilitar</button>`}
          <button id="editar-${item.id}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/tercero/registrosucursal.html?id${item.id}">Editar</a></button>
          <button id="eliminar-${item.id}" class="btn btn-danger btn-sm">Eliminar</button>
          `;
          row.appendChild(cellu);
...
tableBody.addEventListener('click', (event) => {
  if (event.target.id.startsWith('editar-')) {
    ...
    window.location.href = `../../../../../../frontend/public/tercero/registrosucursal.html?id=${userId}`;
...
tableBody.addEventListener('click', (event) => {
  if (event.target.id.startsWith('eliminar-')) {
    ...
    if (confirm(`¿Está seguro de eliminar al usuario con ID ${userId}?`)) {
      ...
      fetch(`http://localhost:3000/sucursal/delete/${userId}`, {
        method: 'GET',
      })
        .then(response => {
          if (response.ok) {
