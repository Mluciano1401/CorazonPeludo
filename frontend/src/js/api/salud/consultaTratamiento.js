...
document.addEventListener('DOMContentLoaded', async() => {
  await populateTable();
     ...
     async function populateTable() {
      try {
        const response = await fetch('http://localhost:3000/tratamiento'); // Replace with your URL
        ...
        data.data.forEach(item => {
          ...
          <button id="editar-${item.id}" class="btn btn-primary btn-sm me-1"><a style="text-decoration: none;" href="../../../../public/enums/registrotratamiento.html?id${item.id}">Editar</a></button>
...
async function busqueda(){
  const urlAPI = "http://localhost:3000/tratamiento"; // URL de la API
...
tableBody.addEventListener('click', (event) => {
  if (event.target.id.startsWith('editar-')) {
    ...
    window.location.href = `../../../../../../frontend/public/admin/registrotratamiento.html?id=${userId}`;
...
tableBody.addEventListener('click', (event) => {
  if (event.target.id.startsWith('eliminar-')) {
    ...
    if (confirm(`¿Está seguro de eliminar al usuario con ID ${userId}?`)) {
      ...
      fetch(`http://localhost:3000/tratamiento/delete/${userId}`, {
        method: 'GET',
      })
        .then(response => {
          if (response.ok) {
